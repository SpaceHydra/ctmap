          ...
          <div className="mt-8 border border-slate-200 rounded-xl overflow-hidden shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <tbody className="bg-white divide-y divide-slate-100">
                {searchResults.map((r) => {
                  // Get owner details if assignment is claimed by someone else
                  const owner = r.ownerId && r.ownerId !== user.id ? store.getUsers().find(u => u.id === r.ownerId) : null;
                  const claimDate = r.auditTrail?.find(log => log.action === 'ASSIGNMENT_CLAIMED')?.timestamp;
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-brand-50/30 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                          ...
                          {/* Show current owner details if claimed by someone else */}
                          {owner && (
                            <div className="mt-2 flex items-center gap-2 text-xs bg-blue-50 border border-blue-100 rounded-lg px-2.5 py-1.5 w-fit">
                              <UserCheck className="w-3.5 h-3.5 text-blue-600 flex-shrink-0" />
                              <div className="flex flex-col">
                                <span className="text-blue-900 font-semibold">Currently owned by: {owner.name}</span>
                                <span className="text-blue-600">Hub: {owner.hubName || 'N/A'} {claimDate ? `• Claimed ${new Date(claimDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}` : ''}</span>
                              </div>
                            </div>
                          )}
                      </td>
